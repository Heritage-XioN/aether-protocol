...
```sh
./target/release/aether-node --dev
```

To purge the development chain's state, run the following command:

```sh
./target/release/aether-node purge-chain --dev
```

To start the development chain with detailed logging, run the following command:

```sh
RUST_BACKTRACE=1 ./target/release/aether-node -ldebug --dev
```
...
$ ./target/release/aether-node --dev --base-path ./my-chain-state/
